...
function waitFor(stream) {
  return new Promise((resolve, reject) => {
    console.log("made it this far 9");
    stream.on('end', resolve);
    console.log("made it this far 10");
    stream.on('error', reject);
    console.log("made it this far 11");
});
}
...
function build() {
  return new Promise((resolve, reject) => { // eslint-disable-line no-unused-vars
    ...
    del([buildDirectory])
      ...
      .catch((error) => { console.warn("some error happened", error);
            reject();
        });
  });
}
...
gulp.task('add-background-sync', function() {
    return gulp.src(['./build/service-worker.js', './src/background-sync-functions.js'])
      .pipe(concat('service-worker.js'))
      .pipe(gulp.dest('./build/'));
  });

gulp.task('default', gulp.series('build', 'add-background-sync'));